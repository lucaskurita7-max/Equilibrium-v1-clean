 ...
 */
function decideMealCount(metabolic, preferencia) {
    // Se a preferência for jejum (Jejum), reduzir refeições
    if (preferencia === 'Jejum')
        return 3;
    // Hipertrofia → mais refeições
    if (metabolic.strategy === 'Bulk')
        return 5;
    // Padrão
    return 4;
}
 ...
 */
function foodsByGroup(group) {
    return foods_1.foods.filter(f => f.grupo === group);
}
/**
 * Seleciona um alimento da lista com índice cíclico.
 */
function pickFood(list, index) {
 ...
 */
function quantityForMacro(targetGrams, foodMacroPer100g) {
    if (foodMacroPer100g <= 0)
        return 0;
 ...
 */
function generateDietPlan(metabolic, preferencias) {
    const mealCount = decideMealCount(metabolic, preferencias === null || preferencias === void 0 ? void 0 : preferencias.preferenciaDieta);
    ...
    for (let i = 0; i < mealCount; i++) {
        ...
        if (carbQty < 0 || !isFinite(carbQty))
            carbQty = 0;
        if (proteinQty < 0 || !isFinite(proteinQty))
            proteinQty = 0;
        if (fatQty < 0 || !isFinite(fatQty))
            fatQty = 0;
        // Construir itens
        const items = [];
        function addItem(food, qty) {
            if (qty <= 0)
                return;
        ...
        const calcTotals = (itms) => itms.reduce((acc, item) => {
            acc.kcal += item.kcal;
