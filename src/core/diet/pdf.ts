...
import { DietPlan } from './dietEngine';
 ...
 */
export async function exportDietPdf(patient: PatientInfo, metabolic: MetabolicOutput, dietPlan: DietPlan): Promise<Buffer> {
  const pages: string[][] = [];
  // Página 1: capa
  pages.push([
  ...
  pages.push([
    'Avaliação',
    '',
    `Peso: ${patient.pesoKg ?? 'N/A'} kg`,
    `Altura: ${patient.alturaM ?? 'N/A'} m`,
  ...
  const mealLines: string[] = ['Plano Alimentar', ''];
  ...
  const subsLines: string[] = ['Substituições Equivalentes', ''];
  ...
  dietPlan.meals.forEach(meal => {
    meal.items.forEach(item => {
      if (!handled.has(item.foodId)) {
        ...
        const eqFoods = getEquivalentFoods(item.foodId);
        if (eqFoods.length > 0) {
