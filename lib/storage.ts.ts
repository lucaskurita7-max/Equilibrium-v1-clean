...
/**
 * Retrieves a single patient by id. Returns undefined if not found. Accepts
 * either a string or numeric id for backward compatibility.
 */
...
export function addPatient(patient: Patient): void {
  ...
  const idx = patients.findIndex((p) => {
    const pid: any = (p as any).id;
    return pid === patient.id || String(pid) === patient.id;
  });
  if (idx >= 0) {
